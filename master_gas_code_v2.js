...
function handleCreateFacility(data) {
  try {
    ...
    try {
      ...
      // テンプレートが入っている親フォルダを取得（B型施設DXフォルダ）
      ...
      if (templateParents.hasNext()) {
        ...
        // 親フォルダがない場合はマイドライブのルート
        parentFolder = DriveApp.getRootFolder();
        Logger.log('Using root folder');
      }

      // 施設名フォルダを作成
      const facilityFolderName = data.facilityName;
      const facilityFolder = parentFolder.createFolder(facilityFolderName);
      facilityFolderId = facilityFolder.getId();
      Logger.log('Facility folder created: ' + facilityFolderName + ' (ID: ' + facilityFolderId + ')');
...
/**
 * テスト用：Drive権限を承認するための関数
 */
function testDrivePermission() {
  try {
    const templateId = '1GSyuEUl_Jn5NaNUcSEIF8oyqN2H9GTAPzDCbb_7C0_o';
    const templateFile = DriveApp.getFileById(templateId);
    const templateName = templateFile.getName();

    Logger.log('成功: テンプレートファイルにアクセスできました');
