...
class _SuperAdminLoginScreenState extends State<SuperAdminLoginScreen> {
  ...
  Future<void> _login() async {
    ...
    try {
      final superAdmin = await _authService.superAdminLogin(email, password);
      ...
      if (mounted) {
        // ログイン成功：全施設管理画面へ（次のフェーズで実装）
        _showSuccessDialog(
          '全権管理者としてログインしました\n\n'
          '管理者名: ${superAdmin.adminName}\n'
          'メール: ${superAdmin.email}\n'
          '権限レベル: ${superAdmin.permissionLevel}',
        );
      }
    } catch (e) {
      if (mounted) {
        ...
        actions: [
          TextButton(
            onPressed: () {
              ...
              Navigator.pop(context); // ログイン画面を閉じてメニューに戻る
            },
            child: const Text('OK'),
          ),
        ],
