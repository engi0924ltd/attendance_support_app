...
const MASTER_SHEET_CONFIG = {
  ...
  FACILITY_COLS: {
    ...
    // 年度別GAS URL（W列〜AJ列: 2026年度〜2039年度）
    GAS_URL_2026: 23,      // W列
    GAS_URL_2027: 24,      // X列
    GAS_URL_2028: 25,      // Y列
    GAS_URL_2029: 26,      // Z列
  ...
  try {
    ...
    switch (endpoint) {
      ...
      case 'facility/update-chatwork-api-key':
        ...
        break;
      case 'facility/update-year-gas-url':
        response = handleUpdateYearGasUrl(data);
        break;
...
/**
 * 年度別GAS URLを更新
 * 施設管理者が次年度GASをデプロイした後にURLを登録する
 */
function handleUpdateYearGasUrl(data) {
  try {
    // バリデーション
    if (!data.facilityId) {
      return createErrorResponse('施設IDが指定されていません');
    ...
    if (!data.fiscalYear) {
      return createErrorResponse('年度が指定されていません');
    }
    if (!data.gasUrl) {
      return createErrorResponse('GAS URLを入力してください');
    }
    ...
    // 年度から列番号を取得
    const yearColumn = getYearGasUrlColumn(data.fiscalYear);
    if (!yearColumn) {
      return createErrorResponse('対応していない年度です（2026〜2039年度のみ対応）');
    }
    ...
    for (let row = MASTER_SHEET_CONFIG.DATA_START_ROW; row <= lastRow; row++) {
      ...
      if (facilityId && facilityId.toString() === data.facilityId.toString()) {
        // 年度別GAS URLを更新
        facilitySheet.getRange(row, yearColumn).setValue(data.gasUrl);

        // アクティブURLも更新する場合（オプション）
        if (data.setAsActive === true) {
          facilitySheet.getRange(row, cols.GAS_URL).setValue(data.gasUrl);
          facilitySheet.getRange(row, cols.FISCAL_YEAR).setValue(data.fiscalYear);
        }
        ...
        return createSuccessResponse({
          message: data.fiscalYear + '年度のGAS URLを登録しました',
          facilityId: facilityId.toString(),
    ...
    return createErrorResponse('年度別GAS URL更新エラー: ' + error.toString());
  }
}

/**
 * 年度から対応する列番号を取得
 * @param {number} fiscalYear - 年度（2026〜2039）
 * @returns {number|null} - 列番号、対応していない年度の場合はnull
 */
function getYearGasUrlColumn(fiscalYear) {
  const year = parseInt(fiscalYear, 10);
  const cols = MASTER_SHEET_CONFIG.FACILITY_COLS;

  const yearColumnMap = {
    2026: cols.GAS_URL_2026,
    2027: cols.GAS_URL_2027,
    2028: cols.GAS_URL_2028,
    2029: cols.GAS_URL_2029,
    2030: cols.GAS_URL_2030,
    2031: cols.GAS_URL_2031,
    2032: cols.GAS_URL_2032,
    2033: cols.GAS_URL_2033,
    2034: cols.GAS_URL_2034,
    2035: cols.GAS_URL_2035,
    2036: cols.GAS_URL_2036,
    2037: cols.GAS_URL_2037,
    2038: cols.GAS_URL_2038,
    2039: cols.GAS_URL_2039
  };

  return yearColumnMap[year] || null;
}
