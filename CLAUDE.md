...
| **出勤済** | 支援記録_YYYYシートにその日の記録がある人数（予定外出勤も含む） | 支援記録シート |
| **未出勤** | 出勤予定者のうちhasCheckedIn=falseの人数 | 計算値 |
| **記録未** | Z列（本人の状況）が未入力の人数 | 支援記録シートZ列 |

**記録未の詳細ロジック:**
1. 出勤予定者で出勤済（hasCheckedIn=true）かつZ列未入力 → カウント
2. 出勤予定者で欠勤・事前連絡あり欠勤かつZ列未入力 → カウント
3. **予定外出勤者（非利用日など）でZ列未入力 → カウント（ステータス問わず）**
4. まだ出勤していない人（記録なし）→ カウントしない

**実装ルール:**
- クイックステータスの計算は**必ず`lib/utils/quick_status_helper.dart`の`calculateQuickStatus()`を使用**
- 施設管理者・支援者画面で別々にロジックを書かない
- 修正時はヘルパー関数を修正し、両画面に自動反映させる
